...
def run(dataset_dir, output_dir, name='std_train', shuffling=False):
    ...
    label_file = dataset_dir + "train_label.txt"
    f = open(label_file, "r")
    ...
    while i < len(label_list):
        ...
        with tf.python_io.TFRecordWriter(tf_filename) as tfrecord_writer:
            j = 0
            while i < len(label_list) and j < SAMPLES_PER_FILES:
                sys.stdout.write('\r>> Converting image %d/%d' % (i+1, len(label_list)))
                sys.stdout.flush()
                # get image bboxes and labels
                bboxes = []
                labels = []
                image_file, obj1, obj2, obj3 = label_list[i].split(',')
                image_file = dataset_dir + image_file[2:]
                objects = [obj1.strip().split(), obj2.strip().split(), obj3.strip().split()]
                shape = [300, 300, 1]
                for obj in objects:
                    labels.append(int(obj[-1]))
                    bboxes.append((float(obj[1])/shape[0],
                                   float(obj[0])/shape[1],
                                   float(obj[3])/shape[0],
                                   float(obj[2])/shape[1]))
